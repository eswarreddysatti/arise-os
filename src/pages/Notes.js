...
export const NotesPage = ({ t, sh, notes, setNotes, userId, onToast }) => {
  ...
  const add = async () => {
    ...
    setAdding(false); setForm({ title: "", content: "" });
    onToast("Note saved");
  };
    ...
    <div className="page" style={{ padding: "70px 20px 100px" }}>
      ...
      {adding ? (
        <Card t={t} sh={sh} style={{ marginBottom: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <NeuTextarea t={t} sh={sh} placeholder="Start writing..." value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} style={{ height: 120 }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {notes.map(n => (
          <Card key={n.id} t={t} sh={sh} style={{ padding: "16px" }}>
            ...
            <p style={{ fontSize: 11, color: t.grey, lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{n.content}</p>
          </Card>
        ))}
      </div>
    </div>
